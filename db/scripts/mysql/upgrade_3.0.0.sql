...
call ExecuteIfColumnExists(Database(), "DELETE_SCHEME", "field_value",
                           "insert into CONFIGURATION_FIELDS (field_key, field_value, configuration_type, configuration_id, index_type)
                           SELECT 'sql_query_killlist', DS.field_value, 'SOURCE', SP.configuration_id, 'DELTA'
                           FROM DELETE_SCHEME AS DS
                           inner JOIN DELTA AS D ON(DS.delete_scheme_id = D.delete_scheme_id)
                           inner join SPHINX_PROCESS as SP on(D.collection_id = SP.collection_id)",
                           "update DELTA set delete_scheme_id=null","delete from DELETE_SCHEME where Type='sql_query_killlist'");
...
create procedure CreateBaseDistributedTemplatesUnlessExists()
  begin
    ...
    THEN
      SELECT @distrTplConf := coalesce(max(configuration_template_id), 0) + 1, @distrTplSearch := coalesce(max(configuration_template_id), 0) + 2 from CONFIGURATION_TEMPLATE;
      ...
      SELECT @fieldId := coalesce(max(configuration_fields_id), 0) + 1 FROM CONFIGURATION_FIELDS;
...
create procedure CreateAdditionalConfigurationTemplateUnlessExists()
  begin
    ...
    THEN
      SELECT @newTplConf := coalesce(max(configuration_template_id), 0) + 1 from CONFIGURATION_TEMPLATE;
