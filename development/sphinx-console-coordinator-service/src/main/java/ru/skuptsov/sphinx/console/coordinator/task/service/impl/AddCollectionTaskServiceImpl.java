...
import ru.skuptsov.sphinx.console.coordinator.task.service.api.AddCollectionTaskService;
import ru.skuptsov.sphinx.console.spring.service.api.CollectionService;

@Service("addCollectionTaskService")
public class AddCollectionTaskServiceImpl extends AbstractTaskService<AddCollectionTask> implements AddCollectionTaskService {
	@Autowired
	...
	@Override
    public synchronized Status handleAgentCallback(AddCollectionTask task, Status status) {
	...
	private void handleProcessing(AddCollectionTask task, Status status) {
		    logger.info("ABOUT TO HANDLE PROCESSING OF ADD COLLECTION STATE, FOR STATUS: " + status + ", STATE: " + task.getState());
