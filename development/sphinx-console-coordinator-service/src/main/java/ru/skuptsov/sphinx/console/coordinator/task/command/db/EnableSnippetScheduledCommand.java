...
@Component("ENABLE_SNIPPET_SCHEDULED")
@Scope("prototype")
public class EnableSnippetScheduledCommand<T extends ProcessTask> extends DbCommand<T> {
	@Autowired
    ...
    public Status execute(ProcessTask task) {
        logger.debug("ENABLE_SNIPPET_SCHEDULED_TASK EXECUTION...");
        ...
        if(scheduledTask != null && task.isSnippetSchedulingEnabledBeforeTaskExecution()) {
